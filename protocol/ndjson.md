...
Zone `id` values are **scoped per registration kind**. The same `id` string may be used independently for a geofence, a corridor, a catalog region, and a radius zone without conflict. Duplicate ids within the same kind are rejected.
