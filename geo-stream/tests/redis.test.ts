...
import { GeoStreamRedis } from '../redis.js'
import type { RedisStreamClient } from '../redis.js'
...
function makeClient(replies: XreadReply[], stopOnExhaust?: GeoStreamRedis): MockRedisClient {
  ...
  return {
    xreadArgs,
    xaddCalls,
    async xread(_countArg, count, _blockArg, blockMs, _streamsArg, stream, lastId) {
...
function reply(entryId: string, fields: Record<string, string>, streamKey = 'location-updates'): XreadReply {
...
describe('GeoStreamRedis — XREAD arguments', () => {
  it('calls xread with correct COUNT / BLOCK / STREAMS / lastId', async () => {
    ...
    adapter = new GeoStreamRedis(makeEngine(), { client, inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM })
  ...
  it('respects batchSize and blockMs options', async () => {
    ...
    adapter = new GeoStreamRedis(makeEngine(), {
      client, inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM,
      batchSize: 50, blockMs: 500,
    })
    const stopClient = makeClient([null], adapter)
    ;(adapter as unknown as { client: RedisStreamClient }).client = stopClient
    await adapter.start()
  ...
  it('respects startId option', async () => {
    ...
    const client = makeClient([null])
    adapter = new GeoStreamRedis(makeEngine(), {
      client, inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM, startId: '0',
    })
    const stopClient = makeClient([null], adapter)
    ;(adapter as unknown as { client: RedisStreamClient }).client = stopClient
    await adapter.start()
...
describe('GeoStreamRedis — message processing', () => {
  it('null xread reply (timeout) → loop continues without calling xadd', async () => {
    ...
    adapter = new GeoStreamRedis(makeEngine([event]), { client: makeClient([]), inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM })
  ...
  it('valid entry → engine.ingest called with correct PointUpdate', async () => {
    ...
    adapter = new GeoStreamRedis(engine, { client: makeClient([]), inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM })
    const stopClient = makeClient([reply('1-1', VALID_FIELDS)], adapter)
    ;(adapter as unknown as { client: RedisStreamClient }).client = stopClient
    await adapter.start()
    assert.equal(engine.ingestCalls.length, 1)
    assert.deepEqual(engine.ingestCalls[0][0], { id: 'v1', x: 1.5, y: 2.5, tMs: 1_700_000_000_000 })
  })
  ...
  it('engine returns no events → xadd not called', async () => {
    let adapter!: GeoStreamRedis
    adapter = new GeoStreamRedis(makeEngine([]), { client: makeClient([]), inputStream: INPUT_STREAM, outputStream: OUTPUT_STREAM })
    const stopClient = makeClient([reply('1-1', VALID_FIELDS)], adapter)
    ;(adapter as unknown as { client: RedisStreamClient }).client = stopClient
    await adapter.start()
  ...
  it('multiple entries in one batch all processed', async () => {
    ...
    const batchReply: XreadReply = [[INPUT_STREAM, [
      ['4-1', flatFields],
      ['4-2', flatFields],
      ['4-3', flatFields],
    ]]]
