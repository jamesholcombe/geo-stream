...
function makeConsumer(): MockConsumer {
  return {
    ...
    async subscribe(opts) {
      this._subscribed.push(opts);
    },
    async run({ eachMessage }) {
      this._eachMessage = eachMessage;
    },
  };
}
...
const VALID_UPDATE: PointUpdate = {
  id: "v1",
  x: 1.5,
  y: 2.5,
...
describe("GeoStreamKafka — lifecycle", () => {
  it("connect() calls connect on both consumer and producer", async () => {
    ...
    const adapter = new GeoStreamKafka(makeEngine(), {
      consumer,
      producer,
