...
export interface RedisStreamClient {
   ...
   */
  xread(
    countArg: 'COUNT', count: number,
    blockArg: 'BLOCK', blockMs: number,
    streamsArg: 'STREAMS', stream: string, lastId: string,
  ): Promise<Array<[string, Array<[string, string[]]>]> | null>

  /**
   * XADD <stream> * <field> <value> [<field> <value> ...]
   */
  xadd(stream: string, id: '*', ...fieldValues: string[]): Promise<string | null>
}
  ...
  private async _loop(): Promise<void> {
    while (this.running) {
      const reply = await this.client.xread(
        ...
        'BLOCK', this.blockMs,
        'STREAMS', this.inputStream, this.lastId,
      )
      ...
      for (const [, entries] of reply) {
        for (const [entryId, rawFields] of entries) {
          ...
          for (const ev of events) {
            const kvPairs = Object.entries(ev).flatMap(([k, v]) => [k, String(v ?? '')])
...
function parseUpdate(fields: Record<string, string>): PointUpdate {
  ...
  if (!isFinite(xn) || !isFinite(yn) || !isFinite(tMs)) {
    throw new Error(`Non-numeric coordinate or timestamp in: ${JSON.stringify(fields)}`)
  }
